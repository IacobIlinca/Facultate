...
void RepositoryLaborator::store(const Medicament &med) {
    const int numar_random = rand() % 10;
    if(numar_random < probability)
        throw Repo_Exception("Mai incearca!");
    Medicament_Repository::store(med);
}


void RepositoryLaborator::stergere_repo(const Medicament &med) {
    const int numar_random = rand() % 10;
    if(numar_random < probability)
        throw Repo_Exception("Mai incearca!");
    Medicament_Repository::stergere_repo(med);
}

void RepositoryLaborator::modificare_pret_repo(const Medicament &med) {
    const int numar_random = rand() % 10;
    if(numar_random < probability)
        throw Repo_Exception("Mai incearca!");
    Medicament_Repository::modificare_pret_repo(med);
}

const vector<Medicament>& RepositoryLaborator::get_all_medicamente() {
    const int numar_random = rand() % 10;
    if(numar_random < probability)
        throw Repo_Exception("Mai incearca!");
    Medicament_Repository::get_all_medicamente();
}

bool RepositoryLaborator::exists(const Medicament &med) {
    const int numar_random = rand() % 10;
    if(numar_random < probability)
        throw Repo_Exception("Mai incearca!");
    Medicament_Repository::exists(med);
}

const Medicament& RepositoryLaborator::find_denumire(string denumire) {
    const int numar_random = rand() % 10;
    if(numar_random < probability)
        throw Repo_Exception("Mai incearca!");
    Medicament_Repository::find_denumire(denumire);
}