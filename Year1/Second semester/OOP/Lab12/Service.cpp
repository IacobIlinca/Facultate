...
Medicament Medicamente_Service::find_dupa_denumire(string denumire) {
    return repo.find_denumire(denumire);
}

void Medicamente_Service::stergere_dupa_denumire(string denumire) {
    const Medicament& med1 = find_dupa_denumire(denumire);
    UndoActions.push_back(make_unique<UndoSterge>(repo, med1));
    repo.stergere_repo(med1);
}
